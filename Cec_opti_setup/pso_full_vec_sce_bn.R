...
PSO_sce_bn_fv_R=function(pop,complexes,dim,xmin,xmax,fitness,gen,printall=T,start_shuffle_prob=0.995){
  library(cec2013)
  fit_func<-function(z){
    switch(fitness,
  ...
  while(k<=gen && all(gbest>=error) && !stuck){
    ...
    wmax=(0.9-0.2)*fac+0.2 # based on Suganthan, Roshida and yoshida et al. #0.9 to 0.4
    #wmin=0.2
